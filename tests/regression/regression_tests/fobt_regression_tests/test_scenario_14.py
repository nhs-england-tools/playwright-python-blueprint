...
def test_scenario_14(page: Page) -> None:
    ...
    SubjectRepository().there_is_letter_batch_for_subject(
        nhs_no, "A348", "GP Letter Indicating Referral to MDT", True
    )
    ...
    HandoverIntoSymptomaticCarePage(page).fill_notes(
        "Handover notes - refer to GP practice"
    )
    ...
    NonNeoplasticResultFromSymptomaticProcedurePage(
        page
    ).enter_date_of_symptomatic_procedure(datetime.today())
    # And the Screening Interval is 36 months
    ...
    SubjectRepository().there_is_letter_batch_for_subject(
        nhs_no, "A374", "Return Surveillance Letter after Referral to Symptomatic", True
    )
    # When I view the advance episode options
    screening_subject_page_searcher.navigate_to_subject_summary_page(page, nhs_no)
    SubjectScreeningSummaryPage(page).click_advance_fobt_screening_episode_button()
    ...
    AdvanceFOBTScreeningEpisodePage(
        page
    ).click_redirect_to_rerecord_the_outcome_of_symptomatic_referral_button()
    # Then my subject has been updated as follows:
    ...
    subject_assertion(
        nhs_no,
        {
            "which diagnostic test": "Latest not-void test in latest episode",
            "latest episode accumulated result": "High-risk findings",
            "latest episode includes event code": "E30 Redirect to Re-record the Outcome of Symptomatic Referral",
            "latest event status": "A372 Refer Symptomatic, GP Letter Printed",
            "symptomatic procedure date": "Null",
            "symptomatic procedure result": "Null",
            "screening referral type": "Null",
