...
def test_lynch_scenario_6_1(page: Page) -> None:
    """
    Scenario: 6.1 - Discharge no contact (no result) - self referral

    G4-G2-G3-A183-A25-J10-A99-A59-A259-A315-A361-A317-A318-A380-A354-A380-A100-A59-A172-A397-A391-A351-C203 Ceased over age [SSCL14b(A351)]

    This scenario tests one of the two possible routes to closure on A351 discharge no contact, following attendance of a "no result" test, then contact is lost before a follow-up test can be arranged.  This test is done for a subject who will be over the Lynch Surveillance age range at recall.
    ...
    > Failed Test - Refer Another > A315 (2.1)
    ...
    > SSP Appointment Required > A380 (2.2)
    > Suitable for Radiological Test > A100 (2.2)
    > Invite for Diagnostic Test > A59 (2.1)
    > Subject DNA > A172 (2.3)
    > Close Episode - No Contact > A397 (2.3)
    > Process A397 letter batch > A391 (2.3)
    > Process A391 letter batch > A351 (2.3)
    > Check recall [SSCL14b(A351)]
    """
    ...
    # And I record contact with the subject with outcome "SSP Appointment Required"
    ContactWithPatientPage(page).record_contact("SSP Appointment Required")

    # Then my subject has been updated as follows:
    subject_assertion(
        nhs_no,
        {
            "latest episode includes event status": "A354 Contact Outcome = SSP Appointment Required",
            "latest event status": "A380 Failed Diagnostic Test - Refer Another",
        },
    )
    ...
    AppointmentAttendance(page).mark_diagnostic_test_as_dna()
    ...
    ContactWithPatientPage(page).record_contact("Close Episode - No Contact", "No")
    ...
    # And there is a "A397" letter batch for my subject with the exact title "Discharge from screening round - no contact (letter to patient) (Lynch)"
    # When I process the open "A397" letter batch for my subject
    batch_processing(
        page,
        "A397",
        "Discharge from screening round - no contact (letter to patient) (Lynch)",
    )
    ...
    # And there is a "A391" letter batch for my subject with the exact title "Discharge from screening round - no contact (letter to GP) (Lynch)"
    # When I process the open "A391" letter batch for my subject
    batch_processing(
        page,
        "A391",
        "Discharge from screening round - no contact (letter to GP) (Lynch)",
    )
    ...
    subject_assertion(
        nhs_no,
        {
            ...
            "lynch incident episode": "Not null",
            "screening due date": "Null",
